...
| Provider registration | `GET/POST /v1/provider/registration*` | **Now implemented** (`ADR-005`, 2026-08-14) — same paths/body shape | ⚠️ **path/shape match; form is missing 2 required fields** (`license_number`, `region_code`) |
...
| Payments | Not started | Not built | **PARTIAL** (aligned-by-absence) | |
...
| `ADR-003` | Status OPEN, but code has already moved past it | **Still open** — this is a product/architecture decision, not something a coding pass can resolve; `provider_dashboard` frozen and labeled `BLOCKED` in the meantime |
...
- **`provider_dashboard` and `lab_booking` remain frozen (`BLOCKED`)** — this pass did not touch either beyond adding an explicit `STATUS.md` warning. Resuming work on them requires a human decision (`ADR-003` resolution, or an explicit reversal of the Laboratory "do not implement" instruction), not more engineering. Continuing to build on an open ADR and an explicit "do not implement" instruction remains the highest-risk activity available in this repository — speculative contract lock-in against domains with no approved architecture.
