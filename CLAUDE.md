...
A separate, self-contained reference — `docs/backend_api_reference_2026-08-25.html` (open in a browser; searchable/filterable) — catalogs all 60 endpoints the `clinic-reservations` backend has shipped through Phase 6, generated directly from its NestJS controllers/DTOs on `main` (2026-08-25), with a wiring badge on every endpoint showing whether `med-super` actually calls it, calls it with a mismatch, or doesn't call it at all. It supersedes an earlier, narrower connectivity-audit doc from 2026-08-21 (removed) and likewise explicitly flags the Phase-3 cap stated above as already stale by the time it was written (Phases 4–6 are done). Treat it as a dated point-in-time snapshot, not a live source of truth — cross-check against current code and `docs/backend_frontend_parity_matrix.md` before relying on any specific claim in it.
