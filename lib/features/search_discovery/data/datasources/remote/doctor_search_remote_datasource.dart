...
class DoctorSearchRemoteDatasource {
  ...
  Future<DoctorSearchResult> searchDoctors({
    ...
    final list = ((data['items'] ?? data['doctors']) as List<dynamic>? ?? const [])
    ...
    return DoctorSearchResult(
      doctors: list,
      totalCount: (data['totalCount'] ?? data['total_count']) as int? ?? list.length,
      nextCursor: (data['nextCursor'] ?? data['next_cursor']) as String?,
    );
  }
